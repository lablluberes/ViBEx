################################
## ViBE'x': A Visualization Tool for Gene Expression Analysis

## Made b'y': Michael Terrefortes-Rosado, Andrea Nieves-Rivera
###############################
...
from hamming import HammingDistance, process_hamming
from networks import create_boolean_network, create_boolean_network_votes, network_rules2, network_rules_vote2
...
def net_tabs(selected_method, data, selected_rows, thr_k, thr_o, thr_s, thr_b, tolerance=4):
    ...
    # return message to dashboard
    if tolerance is None:
        return html.P("Select number of iterations in the interpolation section to generate networks.")
    
    #get the plot, and network of the final vote plot
    fig, final = create_boolean_network_graph_votes(selected_rows, data, selected_method, displacements, thr_k, thr_o, thr_s, thr_b)
    ...
    for method in selected_method:
        ...
        fig2, network = create_boolean_network_graph(selected_rows, data, method, displacements, thr_k, thr_o, thr_s, thr_b)
    ...
    final_transition_df = transition_table(final, "Elected States")
    ...
    return [ dcc.Tabs([
                    ...
                    dcc.Tab(label='Networks', children=[
                        ...
                        #dcc.Tab(label='Elected States', children=[
                    ...
                    #dcc.Tab(label='Transition Rule States', children=[

                    #        html.Div(id='hamming-states')     
                    #])

        
                       
                    #dcc.Tab(label='Final Vote State Table', children=[
                    #      html.P("State table of states for the final vote."),
                    #      html.Br(),
                    #      final_transition_df
                    #])
                ])             
...
@app.callback(
    Output('hamming-states', 'children'),
    Input('datatable-interactivity', 'selected_rows'),
    Input('dropdown-method', 'value'), 
    Input('stored-data','data'),
    Input('stored-rules','data'),
    ...
    Input('thr_o','data'),
    prevent_initial_call=True)
def hamming_states(rows, methods, data, rules, thr_b, thr_k, thr_s, thr_o):
    ...
    for method in methods:
        ...
        network_rule = network_rules2(rows, method, data, rules, thr_b, thr_k, thr_s, thr_o, displacements)

        network = create_boolean_network(rows, method, data, displacements, thr_k, thr_o, thr_s, thr_b)

        table = process_hamming(network_rule, network)

        tables_methods.append(transition_table(table, method))

    network_rule = network_rules_vote2(rows, methods, data, rules, thr_b, thr_k, thr_s, thr_o, displacements)

    network = create_boolean_network_votes(rows, data, methods, displacements, thr_k, thr_o, thr_s, thr_b)

    final_vote_table = transition_table(process_hamming(network_rule, network), "Elected States")
    ...
    if len(methods) > 2:
        ...
        transition_df = html.Div([html.Div(tables_methods[:2]), html.Div(tables_methods[2:])], style={'display': 'flex', 'flexDirection': 'row'})

    else:
        transition_df = html.Div(tables_methods[:2])


    return html.Div([transition_df, final_vote_table], style={'display': 'flex', 'flexDirection': 'row'})
