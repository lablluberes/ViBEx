...
import random, itertools, numpy, math,sys #Pycluster
...
class Binarization(object):	
	...
	'''
	reduces the binary series by removing redundant states and non-determenism (step 2 in paper)
	'''
	...
	def stateTTMostFreq(self, ser):
		...
		# create all possible input state combinations of 1s and 0s: correction, not actually necessary   
		...
		for i,j in inouts.items():
			...
			if len(j) == 0:
				...
				e.sort(key=lambda x: findMostFrequent[x])
...
class KarnaughMaps(object):	
	...
	def getFunction1(self, name, f, func): 
		o = name 
		self.i = f   
		if len(self.i) != 1:
			return ""
	...
	def getFunction2(self, name, f, func):
		o = name 
		self.i = f  
		if len(self.i) != 2:
			return ""
	...
	def getFunction3(self, name, f, func):      
		bin2int = ['00','01','10','11']
		o = name 
		self.i = f   
		if len(self.i) != 3:
	...
	def getBestFitAndError(self, k, colCombinations, binarySeries, order):
		...
		for c in colCombinations: 
			...
			for b in binarySeries:
				...
				seriesLength = len(b[list(b.keys())[0]])
